...
public class InvocableHandlerMethod extends HandlerMethod {
	...
	public Object invokeForRequest(NativeWebRequest request, ModelAndViewContainer mavContainer,
		...
		{//1706
			MethodParameter[] pms = this.getMethodParameters();
			HashMap pmap = new HashMap();
			Map argMap = null;
			for (MethodParameter mp : pms) {
				boolean ignored=false;
				Class<?> ptype = mp.getParameterType();
				if(ptype.isPrimitive())ignored=false;
				else if(ptype.getName().startsWith("javax.servlet")) ignored=true;
				else if(ptype.getName().startsWith("org.springframework")) ignored=true;//TODO ! Model,...,     !!springdata
//				else if(ptype.isAssignableFrom(Map.class) &&
//						mp.getParameterName().startsWith("paramMap")){ 
//					ignored=true; argMap = (Map) args[  mp.getParameterIndex() ];     // paramMap开头的Map参数 但Map特殊
//				}
				//mp.getParameterName()
				if(!ignored){
					pmap.put( mp.getParameterName(), args[ mp.getParameterIndex() ]);
				}else{
				}
			}
			request.setAttribute("_paramMap", pmap, RequestAttributes.SCOPE_REQUEST);
//			if(argMap!=null) argMap.putAll(pmap);
		}
		try{
			...
			request.removeAttribute("_paramMap", RequestAttributes.SCOPE_REQUEST);
		}
	}
